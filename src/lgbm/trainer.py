...
def save_run_artifacts(model, score, feature_names):
...
def prepare_data(df):
    """Selects relevant features without Variety."""
...
def train_brain():
    """Train a default LightGBM on Growth.xlsx and save model into data/."""
    ...
    model.fit(X, y)
    save_run_artifacts(model, accuracy, feature_names)
    print("Training process complete.")

def optimize_and_train():
    """Run a grid search for LightGBM then save the best model."""
    print("--- Starting Hyperparameter Optimization (LightGBM) ---")
    data_path = os.path.join(os.path.dirname(__file__), '..', '..', 'data')
    df = pd.read_excel(os.path.join(data_path, 'Growth.xlsx'))
    ...
    save_run_artifacts(best_model, grid_search.best_score_, feature_names)
